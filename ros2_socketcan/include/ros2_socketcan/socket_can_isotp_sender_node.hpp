...
#ifndef ROS2_SOCKETCAN__SOCKET_CAN_ISOTP_SENDER_NODE_HPP_
#define ROS2_SOCKETCAN__SOCKET_CAN_ISOTP_SENDER_NODE_HPP_
...
namespace socketcan
{
/// \brief SocketCanIsotpSenderNode class which can send ISO-TP messages
/// to CAN hardware or virtual channels
class SOCKETCAN_PUBLIC SocketCanIsotpSenderNode final
  : public lc::LifecycleNode
{
public:
  /// \brief Default constructor
  explicit SocketCanIsotpSenderNode(rclcpp::NodeOptions options);
  ...
  /// \brief Callback for ros ISO-TP frame (using FdFrame message type).
  void on_isotp_frame(const ros2_socketcan_msgs::msg::FdFrame::SharedPtr msg);

private:
  std::string interface_;
  uint32_t tx_id_;
  uint32_t rx_id_;
  rclcpp::Subscription<ros2_socketcan_msgs::msg::FdFrame>::SharedPtr isotp_frames_sub_;
  std::unique_ptr<SocketCanIsotpSender> sender_;
  std::chrono::nanoseconds timeout_ns_;
};
}  // namespace socketcan
}  // namespace drivers

#endif  // ROS2_SOCKETCAN__SOCKET_CAN_ISOTP_SENDER_NODE_HPP_
